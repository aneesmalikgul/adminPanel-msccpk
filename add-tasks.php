...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['btnSaveTask'])) {
    ...
    $taskName = isset($_POST['taskName']) ? $_POST['taskName'] : ''; // Task Name
    $taskDetails = isset($_POST['taskDetails']) ? $_POST['taskDetails'] : ''; // Task Details
    $projectId = isset($_POST['assignProject']) ? (int)$_POST['assignProject'] : 0; // Assigned Project
    $taskPriority = isset($_POST['taskPriority']) ? $_POST['taskPriority'] : ''; // Task Priority
    $taskStatus = isset($_POST['taskStatus']) ? (int)$_POST['taskStatus'] : 1; // Task Status (Active/Inactive)
    $createdBy = $_SESSION['user_id']; // Assuming user ID is stored in session
    $updatedBy = $_SESSION['user_id']; // Same as creator for this example
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['btnUpdateTask'])) {
    ...
    $taskId = isset($_POST['taskId']) ? intval($_POST['taskId']) : 0; // Ensure 'taskId' is being sent through a hidden field
    $projectId = isset($_POST['assignProject']) ? intval($_POST['assignProject']) : 0; // Match with the select field for assigning project
    $taskTitle = isset($_POST['taskName']) ? trim($_POST['taskName']) : ''; // Match with the task name input
    $taskDescription = isset($_POST['taskDetails']) ? trim($_POST['taskDetails']) : ''; // Match with task details input
    $priority = isset($_POST['taskPriority']) ? $_POST['taskPriority'] : 'Medium'; // Match with the task priority dropdown
    $status = isset($_POST['taskStatus']) ? intval($_POST['taskStatus']) : 0; // Match with task status dropdown

    echo  $taskId, $projectId, $taskTitle, $taskDescription, $priority,  $status;
    try {
        // Validate input
